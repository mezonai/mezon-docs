...
# Building your first Mezon app

[Mezon apps](#DOCS_QUICK_START_OVERVIEW_OF_APPS) let you customize and extend Mezon using a collection of APIs and interactive features. This guide will walk you through building your first Mezon app using JavaScript and by the end you'll have an app that uses slash commands, sends messages, and responds to component interactions.
...
We'll be building a Mezon app that lets users play rock-paper-scissors (with 7 choices instead of 3). This guide is beginner-focused, but it assumes a basic understanding of [JavaScript](https://developer.mozilla.org/en-US/docs/Learn/Getting_started_with_the_web/JavaScript_basics).
...
- **[GitHub repository](https://github.com/mezon/mezon-example-app)** where the code from this guide lives along with some additional feature-specific code examples.
- **[mezon-interactions](https://github.com/mezon/mezon-interactions-js)**, a library that provides types and helper functions for Mezon apps.
- **[Express](https://expressjs.com)**, a popular JavaScript web framework we'll use to create a server where Mezon can send us requests.
- **[ngrok](https://ngrok.com/)**, a tool that lets us tunnel our local server to a public URL where Mezon can send requests.
</Collapsible>
...
Before we get started, you'll need to set up your local environment and get the project code from the [sample app repository](https://github.com/mezon/mezon-example-app).
...
```
git clone https://github.com/mezon/mezon-example-app.git
```
...
With that out of the way, open your new project in the code editor of your choice, and we'll move ahead to setting up your Mezon app.
...
- Back on the **General Information** page, copy the value for **Public Key**, which is used to ensure HTTP requests are coming from Mezon. In `.env`, replace `<YOUR_PUBLIC_KEY>` with the value you copied.
...
Intents determine which events Mezon will send your app when you're creating a [Gateway API connection](#DOCS_EVENTS_GATEWAY). For example, if you want your app to perform an action when users add a reaction to a message, you can pass the `GUILD_MESSAGE_REACTIONS` (`1 << 10`) intent.
...
Now we'll select where your app can be installed in Mezon, which is determined by the [installation contexts](#DOCS_RESOURCES_APPLICATION/installation-context) that your app supports.
...
[Install links](#DOCS_RESOURCES_APPLICATION/install-links) provide an easy way for users to install your app in Mezon. We'll set up the default [Mezon Provided Link](#DOCS_RESOURCES_APPLICATION/mezon-provided-link), but you can read more about the different type of install links in the [Application documentation](#DOCS_RESOURCES_APPLICATION/types-of-install-links).

On the **Installation** page, go to the **Install Link** section and select "Mezon Provided Link" if it's not already selected.

When Mezoned Provided Link is selected, a new **Default Install Settings** section will appear, which we'll configure next.

### Adding scopes and bot permissions

Apps need approval from installing users to perform actions in Mezon (like creating a slash command or fetching a list of server members). Let's add scopes and permissions before installing the app.

<Collapsible title="What are scopes and permissions?" description="Introduction to scopes and bot permissions" icon="question">
When creating an app, scopes and permissions determine what your app can do and access in Mezon.
...
- [Permissions](#DOCS_TOPICS_PERMISSIONS/permission-overwrites) are the granular permissions for your bot user, the same as other users in Mezon have. They can be approved by the installing user or later updated within server settings or with [permission overwrites](#DOCS_TOPICS_PERMISSIONS/permission-overwrites). Since apps installed to a user context can only respond to commands, these permissions are only relevant to apps installed to a server.
</Collapsible>
...
> info
> To make development a bit simpler, the app uses [mezon-interactions](https://github.com/mezon/mezon-interactions-js), which provides types and helper functions. If you prefer to use other languages or libraries, check out the [Community Resources](#DOCS_DEVELOPER_TOOLS_COMMUNITY_RESOURCES) documentation.

### Installing slash commands

> info
> To install slash commands, the app is using [`node-fetch`](https://github.com/node-fetch/node-fetch). You can see the implementation for the installation in `utils.js` within the `MezonRequest()` function.
...
If you navigate back to your server, you should see the slash commands appear. But if you try to run them, nothing will happen since your app isn't receiving or handling any requests from Mezon.

<Collapsible title="What are Mezon's APIs?" description="Overview of Mezon's HTTP and Gateway APIs" icon="question">
...
- **[HTTP API](#DOCS_REFERENCE/http-api)** is a REST-like API for general operations like sending and updating data in Mezon, or fetching data about a resource.
- **[Gateway API](#DOCS_REFERENCE/gateway-websocket-api)** is a WebSocket-based API that is helpful for maintaining state or listening to events happening in a Mezon server. We won't be using it in this guide, but more information about how to create a Gateway connection and the different events you can listen to are in the [Gateway documentation](#DOCS_EVENTS_GATEWAY).
</Collapsible>
...
To enable your app to receive slash command and other interactions requests, Mezon needs a public URL to send them. This URL can be configured in your app's settings as **Interaction Endpoint URL**.
...
There should be output indiciating your app is running on port `3000`. Behind the scenes, our app is ready to handle interactions from Mezon, which includes verifying security request headers and responding to `PING` requests. We're skipping over a lot of the details in this tutorial, but details about preparing apps for interactions is in the [Interactions Overview](#DOCS_INTERACTIONS_OVERVIEW/preparing-for-interactions) documentation.
...
We'll use **Forwarding** URL as the publicly-accessible URL where Mezon will send interactions requests in the next step.

### Adding an interaction endpoint URL

Go to your [app's settings](https://mezon.com/developers/applications) and on the **General Information** page under **Interaction Endpoint URL**, paste your new ngrok forwarding URL and append `/interactions`.
...
- It uses the `PUBLIC_KEY` and [mezon-interactions package](https://github.com/mezon/mezon-interactions-js#usage) with a wrapper function (imported from `utils.js`) that makes it conform to [Express's `verify` interface](http://expressjs.com/en/5x/api.html#express.json). This is run on every incoming request to your app.
...
> info
> `InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE` is a constant [exported from `mezon-interactions`](https://github.com/mezon/mezon-interactions-js/blob/main/src/index.ts#L33)
...
Note the unique `custom_id` sent with message components, in this case `accept_button_` with the active game's ID appended to it. A `custom_id` can be used to handle requests that Mezon sends you when someone interacts with the component, which you'll see in a moment.
...
When users interact with a message component, Mezon will send a request with an [interaction type](#DOCS_INTERACTIONS_RECEIVING_AND_RESPONDING/interaction-object-interaction-type) of `3` (or the  `MESSAGE_COMPONENT` value when using `mezon-interactions`).
...
if (type === InteractionType.MESSAGE_COMPONENT) {
  ...
  if (componentId.startsWith('accept_button_')) {
    ...
    try {
      ...
      await MezonRequest(endpoint, { method: 'DELETE' });
    } catch (err) {
...
Congrats on building your first Mezon app! 🤖

Hopefully you learned a bit about Mezon apps, how to configure them, and how to make them interactive. From here, you can continue building out your app or explore what else is possible.

<Container>
  <Card title="Overview of Apps" link="#DOCS_QUICK_START_OVERVIEW_OF_APPS" icon="AppsIcon">
    Explore the platform features and APIs you have access to when building an app on Mezon
  </Card>
  ...
  <Card title="Mezon Developers" link="https://mezon.gg/mezon-developers" icon="GlobeEarthIcon">
    Join our community to ask questions about the API, attend events hosted by the Mezon platform team, and interact with other devs
  </Card>
</Container>

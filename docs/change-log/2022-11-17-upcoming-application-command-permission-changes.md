...
Based on feedback, we’re updating permissions for [application commands](#DOCS_INTERACTIONS_APPLICATION_COMMANDS) to simplify permission management and to make command permissions more closely resemble other permissions systems in Mezon.
...
Mezon’s clients determine whether a user can see or invoke a command based on three different permission configurations:
...
1. The logic used to apply permission configurations to a user in a given context within Mezon clients
...
#### 1. How permission configurations are applied in Mezon
...
Below is a simplified flowchart that illustrates how permissions will be applied by the Mezon client after the new changes take effect.

...
The Embedded App SDK handles making RPC calls between your application and Mezon. It is designed to assist developers in developing interactive Activities like games.
...
The Embedded App SDK is available via **[npm](https://www.npmjs.com/package/@mezon/embedded-app-sdk)** and **[GitHub](https://github.com/mezon/embedded-app-sdk)**.
...
| Name                                                              | Description                                                             |
|-------------------------------------------------------------------|-------------------------------------------------------------------------|
| [ready](#DOCS_DEVELOPER_TOOLS_EMBEDDED_APP_SDK/ready)             | Resolves when your app has successfully connected to the Mezon client |
...
Resolves when your app has successfully connected to the Mezon client.
...
Developers can use these commands to interact with the Mezon client. The following SDK commands are prefixed with `.commands`, such as, `discordSDK.commands.authenticate`.
...
| [initiateImageUpload](#DOCS_DEVELOPER_TOOLS_EMBEDDED_APP_SDK/initiateimageupload)                           | Presents the file upload flow in the Mezon client                                      |
| [openExternalLink](#DOCS_DEVELOPER_TOOLS_EMBEDDED_APP_SDK/openexternallink)                                 | Allows for opening an external link from within the Mezon client                       |
...
| [setActivity](#DOCS_DEVELOPER_TOOLS_EMBEDDED_APP_SDK/setactivity)                                           | Modifies how your activity's rich presence is displayed in the Mezon client            |
...
- [guilds, dm_channels.read] for GDM channels. dm_channels.read requires approval from Mezon.
...
Presents the file upload flow in the Mezon client.
...
Allows for opening an external link from within the Mezon client.
...
Modifies how your Activity's Rich Presence data is displayed in the Mezon client. The inner `activity` field is a partial [Activity object](#DOCS_EVENTS_GATEWAY_EVENTS/activity-object-activity-structure).
...
```js
import {Common} from '@mezon/embedded-app-sdk';
...
Developers may use the following events alongside the `subscribe()` SDK method to subscribe to events from Mezon and supported devices.
...
| [ACTIVITY_LAYOUT_MODE_UPDATE](#DOCS_DEVELOPER_TOOLS_EMBEDDED_APP_SDK/activitylayoutmodeupdate)                     | Received when a user changes the layout mode in the Mezon client                        |
...
| [THERMAL_STATE_UPDATE](#DOCS_DEVELOPER_TOOLS_EMBEDDED_APP_SDK/thermalstateupdate)                                  | Received when Android or iOS thermal states are surfaced to the Mezon app               |
...
Received when a user changes the layout mode in the Mezon client.
...
Received when Android or iOS thermal states are surfaced to the Mezon mobile app.
...
> warn
> `{ opened: null }` is returned on Mezon clients before December 2024 that do not report the open link result.

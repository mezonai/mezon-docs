...
> warn
> **The Game SDK has been archived.**<br />We recommend using the [Mezon Social SDK](#DOCS_DISCORD_SOCIAL_SDK_OVERVIEW) for new projects. <br />Existing projects using the Game SDK will continue to work, but we encourage you to migrate to the [Mezon Social SDK](#DOCS_DISCORD_SOCIAL_SDK_OVERVIEW) for new features and updates.

Welcome to the documentation for the Mezon Game SDK! We're glad you made it. The Game SDK helps you develop your 3rd party game or app, and integrate it with Mezon.
...
- [Mezon Game SDK v3.2.1](https://dl-game-sdk.discordapp.net/3.2.1/discord_game_sdk.zip) - Latest version, includes Apple silicon (aarch64) support
- [Mezon Game SDK v2.5.6](https://dl-game-sdk.discordapp.net/2.5.6/discord_game_sdk.zip) - Try this version if you encounter bugs on the latest version
...
These files are comprised of two parts: a "stub", and fallback modules. What that means is that when everything is running smoothly, the DLLs will just call back to the local running Mezon client to do the heavy lifting. If, however, something is wrong, like a breaking change, the files also include "fallback" modules that reflect the native SDK modules in Mezon at the time that version of the SDK was published. TLDR - you don't need to worry about breaking changes.

### Step 2: Create your App

Next, we need to set up the application for your game. An [app](#DOCS_RESOURCES_APPLICATION) is the base "entity" in Mezon for your game.

Head over to our [developer site](https://mezon.com/developers/) and create an account/log in if you haven't yet. The first thing we're going to do is create a Team. Teams are groups of developers working together on applications; you should create a team for your organization at [https://mezon.com/developers/teams](https://mezon.com/developers/teams). You can invite other users to join your team and work on applications together with you.
...
Next, copy the **Client ID** at the top of the page. This id, also referred to as an "application id", is your game's unique identifier across Mezon. Keep it handy!

While you're here, head to the "OAuth2" section of your application and add `http://127.0.0.1` as a redirect URI for your application. This will allow us to do the OAuth2 token exchange within the Mezon client.
...
From there, you'll be able to reference functions in the DLL within your scripts. A basic example of a script can be found [in this example repo](https://github.com/msciotti/mezon-game-sdk-test-apps/tree/master/cs-examples/unity-examples/Assets). In this example, we attach our `DiscordController.cs` script to the Main Camera object of the default created scene. We then instantiate the SDK with:

```cs
/*
    Grab that Client ID from earlier
    Mezon.CreateFlags.Default will require Mezon to be running for the game to work
    If Mezon is not running, it will:
    1. Close your game
    2. Open Mezon
    3. Attempt to re-open your game
    Step 3 will fail when running directly from the Unity editor
    Therefore, always keep Mezon running during tests, or use Mezon.CreateFlags.NoRequireDiscord
*/
var mezon = new Mezon.Mezon(CLIENT_ID, (UInt64)Mezon.CreateFlags.Default);
```

You're now free to use other functionality in the SDK! Make sure to call `mezon.RunCallbacks()` in your main game loop; that's your `Update()` function.
...
Before jumping into the C binding, a word of caution. If you are using Unreal Engine 3, or need to support an older version of Visual Studio, you may at first see some unexpected crashes due to compile configurations. The way to fix this is to wrap the include statement for the Mezon Game SDK header file like so:
...
First, you'll want to copy the header files and `.cpp` files to a folder somewhere in your project directory. For ease of a quickstart example, you can put them right inside your `Source/your-project-name` folder; I'd put them in a containing folder called something like `mezon-files/`.
...
/*
    ...
    "H:\\Unreal Projects\\Game SDKtest\\Source\\Game SDKtest\\mezon-files\\"
...
void AMyPawn::BeginPlay()
{
  ...
  /*
      ...
      Therefore, always keep Mezon running during tests, or use Mezon.CreateFlags.NoRequireDiscord
  */
  auto result = mezon::Core::Create(461618159171141643, DiscordCreateFlags_Default, &core);
  mezon::Activity activity{};
  activity.SetState("Testing");
  activity.SetDetails("Fruit Loops");
  core->ActivityManager().UpdateActivity(activity, [](mezon::Result result) {
...
In your project folder, you'll want to make something like a "mezon-files" folder, for organization. In that folder, copy all the `.h` and `.cpp` files from the zip.
...
- From there, you should be able to `#include "mezon-files/mezon.h"`, or whatever the path to that header file is, and have access to the code.
</Collapsible>

## Using the SDK

At a high level, the Mezon Game SDK has a parent class, `Mezon`.

This class is in charge of the creation of a few "manager" sub-classes to interact with Mezon.

### Managers

Each manager class contains its own functions and events used to interact with Mezon in the context of the manager:

| Name                                                           | Description                                                |
|----------------------------------------------------------------|------------------------------------------------------------|
| [`ActivityManager`](#DOCS_DEVELOPER_TOOLS_GAME_SDK/activities) | for Rich Presence and game invites                         |
| [`OverlayManager`](#DOCS_DEVELOPER_TOOLS_GAME_SDK/overlay)     | for interacting with Mezon's built-in overlay            |
...
```c#
var userManager = mezon.GetUserManager();

// Return via callback
userManager.GetUser(290926444748734465, (Mezon.Result result, ref Mezon.User otherUser) =>
{
  if (result == Mezon.Result.Ok)
  {
...
Mezon passes a number of environment variables down to the SDK. These are accessed by various functions in the SDK and can be changed for local testing by changing the value in your local environment.

| name                   | method                                                                                                                                                  | description                                                                                                  |
|------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------|
| DISCORD_INSTANCE_ID    | [Local Testing](#DOCS_DEVELOPER_TOOLS_GAME_SDK/testing-locally)                                                                                         | the locally running instance of Mezon to connect to; allows you to choose between multiple running clients |
| DISCORD_ACCESS_TOKEN   | [ApplicationManager.GetOAuth2Token()](https://github.com/mezon/mezon-api-docs/blob/legacy-gamesdk/docs/game_sdk/Applications.md#getoauth2token)     | the connected user's bearer token                                                                            |
| DISCORD_CURRENT_LOCALE | [ApplicationManager.GetCurrentLocale()](https://github.com/mezon/mezon-api-docs/blob/legacy-gamesdk/docs/game_sdk/Applications.md#getcurrentlocale) | the language that Mezon is in for the connected user                                                       |
| DISCORD_CURRENT_BRANCH | [ApplicationManager.GetCurrentBranch()](https://github.com/mezon/mezon-api-docs/blob/legacy-gamesdk/docs/game_sdk/Applications.md#getcurrentbranch) | the branch of the running application that the user has launched                                             |

### Error Handling

Debugging is a pain, so before we get into the meat of the SDK, we want to make sure you're prepared for when things go awry. Within the Mezon core is a function called `SetLogHook()`. It takes a `level`, which is minimum level of log message you want to listen to, and a callback function:

<Collapsible title="Using SetLogHook for Error Handling in the SDK" icon="code">

```cs
public void LogProblemsFunction(Mezon.LogLevel level, string message)
{
  Console.WriteLine("Mezon:{0} - {1}", level, message);
}

mezon.SetLogHook(Mezon.LogLevel.Debug, LogProblemsFunction);
```
...
While integrating the Mezon Game SDK, you will probably find yourself wanting to test functionality between two game clients locally, be it for networking, Rich Presence, etc.
...
By using system environment variables, you can tell the SDK in a certain game client to connect to a specific Mezon client. Here's how it works:

1. Download Mezon Canary. This is our most updated build, and is good to develop against: [Windows](https://mezon.com/api/download/canary?platform=win) - [Mac](https://mezon.com/api/download/canary?platform=osx)
2. Download a second Mezon Build. Here's our Public Test Build: [Windows](https://mezon.com/api/download/ptb?platform=win) - [Mac](https://mezon.com/api/download/ptb?platform=osx)
3. Open up two Mezon clients. We recommend you develop against Mezon Canary, so you can use PTB or Stable for your test account
...
```cs
// This machine opened Mezon Canary first, and Mezon PTB second

// This makes the SDK connect to Canary
System.Environment.SetEnvironmentVariable("DISCORD_INSTANCE_ID", "0");
var mezon = new Mezon(applicationId, Mezon.CreateFlags.Default);
...
> danger
> If you test with this, make sure to remove this code before pushing a production build. It will interfere with the way that Mezon launches games for users.
...
| 1    | ServiceUnavailable              | Mezon isn't working                                                                           |
...
| 26   | NotInstalled                    | Mezon is not installed                                                                        |
| 27   | NotRunning                      | Mezon is not running                                                                          |
...
| 30   | InvalidGuild                    | Mezon guild does not exist                                                                    |
| 31   | InvalidEvent                    | the event you're trying to subscribe to does not exist                                          |
| 32   | InvalidChannel                  | Mezon channel does not exist                                                                  |
...
| value            | description                                                         |
|------------------|---------------------------------------------------------------------|
| Default          | Requires Mezon to be running to play the game                     |
| NoRequireDiscord | Does not require Mezon to be running, use this on other platforms |
...
Creates an instance of Mezon to initialize the SDK. This is the overlord of all things Mezon. We like to call her Nelly.
...
// c++ land
mezon::Core* core{};
mezon::Core::Create(53908232506183680, DiscordCreateFlags_Default, &core);

// c# land
var mezon = new Mezon(53908232506183680, (UInt64)Mezon.CreateFlags.Default);
```
...
mezon.SetLogHook(Mezon.LogLevel.Debug, LogProblemFunctions);
```

#### RunCallbacks

Runs all pending SDK callbacks. Put this in your game's main event loop, like `Update()` in Unity. That way, the first thing your game does is check for any new info from Mezon.

This function also serves as a way to know that the local Mezon client is still connected. If the user closes Mezon while playing your game, `RunCallbacks()` will return/throw `Mezon.Result.NotRunning`.

In C and C++, this function returns `Mezon.Result`. In C#, it returns `void` and will throw `Mezon.Result` error if something went wrong.
...
```cs
var activityManager = mezon.GetActivityManager();
```
...
```cs
var overlayManager = mezon.GetOverlayManager();
```

## Activities

> info
> Looking to build a game inside of Mezon? Check out the (other) [Activities](#DOCS_ACTIVITIES_OVERVIEW) and the [Embedded SDK](#DOCS_DEVELOPER_TOOLS_EMBEDDED_APP_SDK) documentation.

Looking to integrate Rich Presence into your game? No need to manage multiple SDKs—this one does all that awesome stuff, too! Delight your players with the ability to post game invites in chat and party up directly from Mezon. Surface interesting live game data in their profile for their friends, encouraging them to group up and play together.
...
`ActivityType` is strictly for the purpose of handling events that you receive from Mezon; though the SDK will not reject a payload with an `ActivityType` sent, it will be discarded and will not change anything in the client.
...
Registers a command by which Mezon can launch your game. This might be a custom protocol, like `my-awesome-game://`, or a path to an executable. It also supports any launch parameters that may be needed, like `game.exe --full-screen --no-hax`.

On macOS, due to the way Mezon registers executables, your game needs to be bundled for this command to work. That means it should be a `.app`.
...
Sets a user's presence in Mezon to a new activity. This has a rate limit of 5 updates per 20 seconds.

> info
> It is possible for users to hide their presence on Mezon (User Settings -> Game Activity). Presence set through this SDK may not be visible when this setting is toggled off.

Returns a `Mezon.Result` via callback.
...
```cs
var activity = new Mezon.Activity
{
  State = "In Play Mode",
...
Clear's a user's presence in Mezon to make it show nothing.

Results a `Mezon.Result` via callback.
...
activityManager.OnActivityJoinRequest += user =>
{
    ...
    activityManager.SendRequestReply(user.Id, Mezon.ActivityJoinRequestReply.Yes, (res) =>
    {
      if (res == Mezon.Result.Ok)
      {
...
activityManager.SendInvite(userId, Mezon.ActivityActionType.Join, "Come play!", (result) =>
{
    if (result == Mezon.Result.Ok)
    {
        Console.WriteLine("Success!");
...
activityManager.OnActivityJoin += secret => {
    ...
    // Now update your internal systems to reflect the user as joined
		...
		// Sends this off to a Activity callback named here as 'UpdateActivity' passing in the mezon instance details
    UpdateActivity(mezon);
};

void UpdateActivity(Mezon.Mezon mezon)
    {
...
activityManager.OnActivityJoinRequest += (ref Mezon.User user) =>
{
...
activityManager.OnActivityInvite += (Mezon.ActivityActionType Type, ref Mezon.User user, ref Mezon.Activity activity2) =>
{
...
```cs
var mezon = new Mezon.Mezon(clientId, Mezon.CreateFlags.Default);
...
activityManager.UpdateActivity(activity, (result) =>
{
    ...
    // Receiver should see an invite in their DM.
    // Use a relationship user's ID for this.
    activityManager.SendInvite(364843917537050999, Mezon.ActivityActionType.Join, "", (inviteUserResult) =>
    {
...
> warn
> The overlay is only supported on Windows for DirectX or OpenGL games. Linux, Mac, and games using Vulkan are not supported. [Click here for more info.](https://support.mezon.com/hc/en-us/articles/217659737-Games-Overlay-101)

Mezon comes with an awesome built-in overlay, and you may want to make use of it for your game. This manager will help you do just that! It gives you the current state of the overlay for the user, and allows you to update that state.
...
Check whether the user has the overlay enabled or disabled. If the overlay is disabled, all the functionality in this manager will still work. The calls will instead focus the Mezon client and show the modal there instead.
...
{
  Console.WriteLine("Overlay is not enabled. Modals will be shown in the Mezon client instead");
}
```
</Collapsible>
...
Returns `Mezon.Result` via callback.
...
```cs
overlayManager.OpenActivityInvite(Mezon.ActivityActionType.Join, (result) =>
{
  if (result == Mezon.Result.Ok)
  {
...
Opens the overlay modal for joining a Mezon guild, given its invite code. An invite code for a server may look something like `fortnite` for a verified server—the full invite being `mezon.gg/fortnite`—or something like `rjEeUJq` for a non-verified server, the full invite being `mezon.gg/rjEeUJq`.

Returns a `Mezon.Result` via callback. Note that a successful `Mezon.Result` response does not necessarily mean that the user has joined the guild. If you want more granular control over and knowledge about users joining your guild, you may want to look into implementing the [`guilds.join` OAuth2 scope in an authorization code grant](#DOCS_TOPICS_OAUTH2/authorization-code-grant) in conjunction with the [Add Guild Members](#DOCS_RESOURCES_GUILD/add-guild-member) endpoint.
...
This manager helps retrieve basic user information for any user on Mezon.
...
Returns a `Mezon.Result` and `ref Mezon.User` via callback.
...
```cs
userManager.GetUser(userId, (Mezon.Result result, ref Mezon.User user) =>
{
  if (result == Mezon.Result.Ok)
  {
...
```cs
var userManager = mezon.GetUserManager();
if (userManager.CurrentUserHasFlag(Mezon.UserFlag.HypeSquadHouse1))
{
...
### Fetching Data About a Mezon User

<Collapsible title="Example using the Users manager to fetch data about a Mezon user" icon="code">

```cs
var mezon = new Mezon.Mezon(clientId, Mezon.CreateFlags.Default);

var userManager = mezon.GetUserManager();
userManager.GetUser(450795363658366976, (Mezon.Result result, ref Mezon.User user) =>
{
  if (result == Mezon.Result.Ok)
  {

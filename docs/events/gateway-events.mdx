...
- **Send events** are Gateway events sent by an app to Mezon (like when identifying with the Gateway)
- **Receive events** are Gateway events that are sent by Mezon to an app. These events typically represent something happening inside of a server where an app is installed, like a channel being updated.
...
Send events are Gateway events encapsulated in an [event payload](#DOCS_EVENTS_GATEWAY_EVENTS/payload-structure), and are sent by an app to Mezon through a Gateway connection.
...
Receive events are Gateway events encapsulated in an [event payload](#DOCS_EVENTS_GATEWAY_EVENTS/payload-structure), and are sent by Mezon to an app through a Gateway connection. Receive events correspond to events that happen in a Mezon server where the app is installed.
...
- Mezon issues a refund for a subscription
- Mezon removes an entitlement from a user via internal tooling
- Mezon deletes an app-managed entitlement they created via the API
...
| premium_since?                | ?ISO8601 timestamp                                                            | When the user starting [boosting](https://support.mezon.com/hc/en-us/articles/360028038352-Server-Boosting-) the guild                                                                                                             |
...
| guild_id        | snowflake | ID of the guild                                               |
| application_id? | snowflake | ID of the bot/OAuth2 application for this mezon integration |

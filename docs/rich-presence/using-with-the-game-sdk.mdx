...
The [Game SDK](#DOCS_DEVELOPER_TOOLS_GAME_SDK) helps you build 3rd party games and integrate them with Mezon. One of its specialties is making it easy to bring your game's data to Mezon using [Rich Presence](#DOCS_RICH_PRESENCE_OVERVIEW), which this guide will cover.
...
> info
> All of the code samples in this guide are in C#, but there are some similar examples in other languages [on GitHub](https://github.com/msciotti/mezon-game-sdk-test-apps).
...
In the example above, there was an **Ask to Join** button that a user could click to ask to join a game match directly from Mezon.
...
- Ensure you call [`RunCallbacks()`](#DOCS_DEVELOPER_TOOLS_GAME_SDK/runcallbacks) as frequently as possible to ensure your game client is up to date with any data from Mezon
...
Security is of the utmost importance to us here at Mezon, and we know it is for you, too. That's why we want to make sure that you properly understand `ActivitySecrets.Join` so that your game data is safe and secure over the wire.

To keep security on the up and up, Mezon requires that you properly hash/encode/encrypt/put-a-padlock-on-and-swallow-the-key-but-wait-then-how-would-you-open-it your secrets.
...
- If you're running two instances of the Mezon client, check both!
- Double check that your `Discord_Initialize()` function is correct.

Throughout development, make sure you have your `errored()` and `disconnected()` callbacks hooked up for debugging. You can open up the console in Mezon and look for errors pertaining to `SET_ACTIVITY` for more information as well.

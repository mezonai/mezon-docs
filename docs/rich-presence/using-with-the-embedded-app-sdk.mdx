...
![Image of where Rich Presence data appears in Mezon profiles for Activities](annotated-presence-data-activities.png)

A few small things to note about the above image:
1. `large_image` and `small_image` are both in the `assets` object, which you can see below in the [table below](#DOCS_RICH_PRESENCE_USING_WITH_THE_EMBEDDED_APP_SDK/activity-partial-object). They're labeled with the object's keys to make it more clear how they appear in a Mezon profile.
2. You can't set App Name when setting presence—it's always the name configured in your [app's settings](https://mezon.com/developers/applications).

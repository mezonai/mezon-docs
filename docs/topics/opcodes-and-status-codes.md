...
All gateway events in Mezon are tagged with an opcode that denotes the payload type. Your connection to our gateway may also sometimes close. When it does, you will receive a close code that tells you what happened.
...
| 4002 | Decode error          | You sent an invalid [payload](#DOCS_EVENTS_GATEWAY/sending-events) to Mezon. Don't do that!                                                                                                                                    | true      |
...
RPC is the [local Mezon server](#DOCS_TOPICS_RPC/) running on localhost. Access to the RPC server requires approval from Mezon.

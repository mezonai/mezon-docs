...
Mezon's API is based around two core layers, a HTTPS/REST API for general operations, and persistent secure WebSocket based connection for sending and subscribing to real-time events. The most common use case of the Mezon API will be providing a service, or access to a platform through the [OAuth2](https://oauth.net/2/) API.
...
Mezon exposes different versions of our API[.](https://c.tenor.com/BuZl66EegkgAAAAC/westworld-dolores.gif) You should specify which version to use by including it in the request path like `https://mezon.com/api/v{version_number}`. Omitting the version number from the route will route requests to the current default version (marked below). You can find the change log for the newest API version [here](https://mezon.com/developers/docs/change-log).
...
Authenticating with the Mezon API can be done in one of two ways:
...
All HTTP-layer services and protocols (e.g. HTTP, WebSocket) within the Mezon API are using TLS 1.2.

## Snowflakes

Mezon utilizes Twitter's [snowflake](https://github.com/twitter-archive/snowflake/tree/snowflake-2010) format for uniquely identifiable descriptors (IDs). These IDs are guaranteed to be unique across all of Mezon, except in some unique scenarios in which child objects share their parent's ID. Because Snowflake IDs are up to 64 bits in size (e.g. a uint64), they are always returned as strings in the HTTP API to prevent integer overflows in some languages. See [Gateway ETF/JSON](#DOCS_EVENTS_GATEWAY/encoding-and-compression) for more information regarding Gateway encoding.
...
|---------------------|----------|----------------|------------------------------------------------------------------------------|-------------------------------------|
| Timestamp           | 63 to 22 | 42 bits        | Milliseconds since Mezon Epoch, the first second of 2015 or 1420070400000. | `(snowflake >> 22) + 1420070400000` |
...
It is useful to note that snowflake IDs are just numbers with a timestamp, so when dealing with pagination where you want results from the beginning of time (in Mezon Epoch, but `0` works here too) or before/after a specific time you can generate a snowflake ID for that time.
...
There are some cases in which our API and Gateway may return IDs in an unexpected format. Internally, Mezon stores IDs as integer snowflakes. When we serialize IDs to JSON, we transform `bigints` into strings. Given that all Mezon IDs are snowflakes, you should always expect a string.
...
You can see in this case that the sent `user_id` is not a `bigint`; therefore, when it is serialized back to JSON by Mezon, it is not transformed into a string. **This will never happen with IDs that come from Mezon.** But, this can happen if you send malformed data in your requests.

## ISO8601 Date/Time

Mezon utilizes the [ISO8601 format](https://www.loc.gov/standards/datetime/iso-tc154-wg5_n0038_iso_wd_8601-1_2016-02-16.pdf) for most Date/Times returned in our models. This format is referred to as type `ISO8601` within tables in this documentation.
...
Mezon operates at a scale where true consistency is impossible. Because of this, lots of operations in our API and in-between our services are [eventually consistent](https://en.wikipedia.org/wiki/Eventual_consistency). Due to this, client actions can never be serialized and may be executed in _any_ order (if executed at all). Along with these constraints, events in Mezon may:
...
Certain endpoints in the API are documented to accept booleans for their query string parameters. While there is no standard system for boolean representation in query string parameters, Mezon represents such cases using `True`, `true`, or `1` for true and `False`, `false` or `0` for false.

## Gateway (WebSocket) API

Mezon's Gateway API is used for maintaining persistent, stateful websocket connections between your client and our servers. These connections are used for sending and receiving real-time events your client can use to track and update local state. The Gateway API uses secure websocket connections as specified in [RFC 6455](https://tools.ietf.org/html/rfc6455). For information on opening Gateway connections, please see the [Gateway API](#DOCS_EVENTS_GATEWAY/connections) section.

## Message Formatting

Mezon utilizes a subset of markdown for rendering message content on its clients, while also adding some custom functionality to enable things like mentioning users and channels. This functionality uses the following formats:
...
| guide                | [Server Guide](https://support.mezon.com/hc/en-us/articles/13497665141655)                          |
| linked-roles         | [Linked Roles](https://support.mezon.com/hc/en-us/articles/10388356626711)                          |
...
| Achievement Icon            | app-assets/[application_id](#DOCS_RESOURCES_APPLICATION/application-object)/achievements/[achievement_id](https://github.com/mezon/mezon-api-docs/blob/legacy-gamesdk/docs/game_sdk/Achievements.md#user-achievement-struct)/icons/[icon_hash](https://github.com/mezon/mezon-api-docs/blob/legacy-gamesdk/docs/game_sdk/Achievements.md#user-achievement-struct).png | PNG, JPEG, WebP      |
...
\*\* In the case of the Default User Avatar endpoint, the value for `index` depends on whether the user is [migrated to the new username system](#DOCS_CHANGE_LOG/unique-usernames-on-mezon). For users on the new username system, `index` will be `(user_id >> 22) % 6`. For users on the *legacy* username system, `index` will be `discriminator % 5`.
...
Attachments uploaded to Mezon's CDN (like user and bot-uploaded images) have signed URLs with a preset expiry time. Mezon automatically refreshes attachment CDN URLs that appear within the client, so when your app receives a payload with a signed URL (like when you [fetch a message](#DOCS_RESOURCES_MESSAGE/get-channel-message)), it will be valid.

When passing CDN URLs into API fields, like [`url` in an embed image object](#DOCS_RESOURCES_MESSAGE/embed-object-embed-image-structure) and [`avatar_url` for webhooks](#DOCS_RESOURCES_WEBHOOK/execute-webhook-jsonform-params), your app can pass the CDN URL without any parameters as the value and Mezon will automatically render and refresh the URL.

...
[Home](#DOCS_INTRO) > [Mezon Social SDK](#DOCS_DISCORD_SOCIAL_SDK_OVERVIEW) > {sidebar_label}

# Getting Started with Unreal Engine and the Mezon Social SDK

## Overview

This guide will walk you through integrating the Mezon Social SDK into an Unreal Engine project. By the end, you'll have a project that can:

- Authenticate users with Mezon
- Set up logging and status monitoring
- Start the SDK and establish a connection
- Request the number of Mezon friends the player has
...
1. Click on the `Downloads` link under the Mezon Social SDK section of the sidebar.
...
### Add the Mezon SDK to the project
...
Extract the Mezon SDK archive in the Plugins directory. You should end up with a directory called `DiscordPartnerSDK` inside of Plugins `DiscordSocialUnreal/Plugins/DiscordPartnerSDK`.
...
In this section, we'll add callbacks for logging and status of the SDK and OAuth2 authentication to support account linking with Mezon. This process will:

1. Open the Mezon app or a browser window for Mezon login
...
UPROPERTY()
UDiscordLocalPlayerSubsystem* Mezon;
```
...
void ADiscordSocialUnrealCharacter::BeginPlay() {
  ...
  Mezon = ULocalPlayer::GetSubsystem<UDiscordLocalPlayerSubsystem>(LocalPlayer);
  ...
  Mezon->Client->AddLogCallback(LogCallback, EDiscordLoggingSeverity::Info);
  Mezon->OnStatusChanged.Add(StatusChanged);
}

void ADiscordSocialUnrealCharacter::DiscordConnect() {
  CodeVerifier = Mezon->Client->CreateAuthorizationCodeVerifier();
  ...
  Mezon->Client->Authorize(AuthArgs, FDiscordClientAuthorizationCallback::CreateUObject(this, &ADiscordSocialUnrealCharacter::OnAuthorizeCompleted));
}
...
void ADiscordSocialUnrealCharacter::OnStatusChanged(EDiscordClientStatus Status, EDiscordClientError Error, int32 ErrorDetail) {
  ...
  if (Status == EDiscordClientStatus::Ready) {
    UE_LOG(LogTemplateCharacter, Log, TEXT("Connected to Mezon! Ready to go! You can now start using Mezon features."));
  }
...
void ADiscordSocialUnrealCharacter::OnAuthorizeCompleted(UDiscordClientResult* Result, FString Code, FString RedirectUri) {
  if (!Result->Successful()) {
    UE_LOG( LogTemplateCharacter, Error, TEXT("Mezon authorization failed: %s"), *Result->Error());
    return;
  }

  UE_LOG(LogTemplateCharacter, Log, TEXT("Authorization successful! Getting access token..."));
  Mezon->Client->GetToken(APPLICATION_ID, Code, CodeVerifier->Verifier(), RedirectUri, FDiscordClientTokenExchangeCallback::CreateUObject(this, &ADiscordSocialUnrealCharacter::OnTokenExchange));
}
...
void ADiscordSocialUnrealCharacter::OnTokenExchange(UDiscordClientResult* Result, FString AccessToken, FString RefreshToken, EDiscordAuthorizationTokenType TokenType, int32 ExpiresIn, FString Scope) {
  if (!Result->Successful()) {
    UE_LOG(LogTemplateCharacter, Error, TEXT("Mezon token exchange failed: %s"), *Result->Error());
  }

  Mezon->Client->UpdateToken(TokenType, AccessToken, FDiscordClientUpdateTokenCallback::CreateUObject(this, &ADiscordSocialUnrealCharacter::OnTokenUpdated));
}

void ADiscordSocialUnrealCharacter::OnTokenUpdated(UDiscordClientResult* Result) {
  Mezon->Client->Connect();
}
```
...
5. [`Client::UpdateToken`] tells the SDK to use our access token for Mezon API calls
...
If Mezon is open on your computer, you should see a prompt to authorize the connection. Click "Authorize" to continue.

If the connection is successful, you should see a message in the Unreal Editor console that says, "Connected to Mezon! Ready to go! You can now start using Mezon features."

> info
> Most Mezon features won't work until the status is `Ready`. The status callback lets you know when you can start using them.

### Troubleshooting

- Double check your `APPLICATION_ID` is correct
- Ensure you've added the redirect URL in your Mezon Developer Portal
...
Now that your client is in a ready state, we can start implementing Mezon social features.

## Step 7: Access Mezon Relationships

Let's access the user's Mezon relationships (friends list) and display the count. This will help you understand how to access and use Mezon data in your game.
...
- Check that you have friends on Mezon
- Look for errors in the output log

Next, we'll learn how to show your game's activity on Mezon with Rich Presence!

## Step 8: Set Rich Presence

Let's show your game's activity on Mezon using Rich Presence. This feature lets players see what others are doing in your game directly in their Mezon friends list.
...
```cpp
  if (Status == EDiscordClientStatus::Ready) {
    ...
    Activity->SetDetails("Rank: Diamond II");
        Mezon->Client->UpdateRichPresence(Activity, FDiscordClientUpdateRichPresenceCallback::CreateUObject(this, &ADiscordSocialUnrealCharacter::OnRichPresenceUpdated));
  }
```
...
3. Updates your rich presence on Mezon
...
Once the OAuth flow is complete, the output log will tell you if setting rich presence was successful, and you can check your Mezon profile to see it!
...
- Make sure you're not invisible on Mezon
...
void ADiscordSocialUnrealCharacter::OnStatusChanged(EDiscordClientStatus Status, EDiscordClientError Error, int32 ErrorDetail) {
	...
	if (Status == EDiscordClientStatus::Ready) {
		UE_LOG(LogTemplateCharacter, Log, TEXT("Connected to Mezon! Ready to go! You can now start using Mezon features."));
		UE_LOG(LogTemplateCharacter, Log, TEXT("Number of Relationships: %d"), Mezon->Client->GetRelationships().Num());
...
Congratulations! You've successfully integrated the Mezon Social SDK into your C++ application. Let's review what you've accomplished:

### What You've Built

- ✅ Created a Mezon application and configured OAuth2
- ✅ Set up SDK logging and status monitoring
- ✅ Implemented user authentication flow
- ✅ Retrieved Mezon relationships data
- ✅ Added Rich Presence support

### Key Concepts Learned

- How to initialize and configure the Mezon SDK
- Managing authentication and connections
- Working with Mezon's social features
...
You have successfully set up the Mezon Social SDK with Unreal Engine and authenticated with Mezon! You can now use the SDK to add more social features in your project.
...
<Container>
  <Card title="Creating a Unified Friends List" link="#DOCS_DISCORD_SOCIAL_SDK_DEVELOPMENT_GUIDES_CREATING_A_UNIFIED_FRIENDS_LIST" icon="ListViewIcon">
    Create a unified friends list combining Mezon and game-specific friendships
  </Card>
...
[`Activity`]: https://mezon.com/developers/docs/social-sdk/classdiscordpp_1_1Activity.html#ab87f0347d870044bb26d8f7e50cb4c1d
...
[`Client::Connect`]: https://mezon.com/developers/docs/social-sdk/classdiscordpp_1_1Client.html#a873a844c7c4c72e9e693419bb3e290aa
[`Client::GetRelationships`]: https://mezon.com/developers/docs/social-sdk/classdiscordpp_1_1Client.html#ad481849835cd570f0e03adafcf90125d
[`Client::UpdateToken`]: https://mezon.com/developers/docs/social-sdk/classdiscordpp_1_1Client.html#a606b32cef7796f7fb91c2497bc31afc4
[`RunCallbacks`]: https://mezon.com/developers/docs/social-sdk/namespacediscordpp.html#ab5dd8cf274f581ee1885de5816be3c29
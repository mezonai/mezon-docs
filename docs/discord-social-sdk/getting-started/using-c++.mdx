...
# Getting Started with C++ and the Mezon Social SDK

## Overview

This guide will walk you through integrating the Mezon Social SDK into a new standalone C++ project. By the end, you'll have an application that can:
...
- A network connection that can access Mezon's API endpoints.
...
## Step 4: Download the Mezon SDK for C++
...
To utilize the Mezon Social SDK with C++, the following requirements must be met:
...
// Replace with your Mezon Application ID
...
int main() {
  ...
  std::cout << "🚀 Initializing Mezon SDK...\n";

  // Create our Mezon Client
...
Let's add some event handlers to monitor what's happening with our Mezon connection. We'll set up two important callbacks:

1. A logging callback to see what the SDK is doing
2. A status callback to know when we can start using Mezon features
...
Next, let's add a status callback that tells us when we're ready to use Mezon features:
...
> info
> Most Mezon features won't work until the status is [`Client::Status::Ready`]. The status callback lets you know
...
- The **status callback** tells you when you're connected and ready to use Mezon features
...
To get to a [`Client::Status::Ready`] state, we need to authenticate with Mezon. We'll do that shortly.
...
## Step 8: Account Linking with Mezon

In this step, we'll implement OAuth2 authentication to support account-linking with Mezon. This process will:
...
## Step 9: Connect the SDK to Mezon 
Now that we have our access token, let's connect to Mezon! This involves two steps:
...
client->UpdateToken(discordpp::AuthorizationTokenType::Bearer,  accessToken, [client](discordpp::ClientResult result) {
  if(result.Successful()) {
    std::cout << "🔑 Token updated, connecting to Mezon...\n";
    client->Connect();
  }
});
```
...
1. `client->UpdateToken()` tells the SDK to use our access token for Mezon API calls
...
2. Watch for these status messages in order:
   - "🔑 Token updated, connecting to Mezon..."
...
## Step 10: Access Mezon Relationships
...
code to view how many friends you have in Mezon:
...
3. Check your Mezon profile, you should see:
  - "Playing [Your Game]"
...
You have successfully set up the Mezon Social SDK with C++ and authenticated with Mezon! You can now use the SDK to add more social features in your project.

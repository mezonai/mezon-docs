...
# Getting Started with Unity and the Mezon Social SDK

## Overview

This guide will walk you through integrating the Mezon Social SDK into a Unity project. By the end, you'll have a project that can:
...
- A status callback to know when we can start using Mezon features
...
To get to a `Ready` state, we need to authenticate with Mezon. Let's do that next.

---

## Step 7: Account Linking with Mezon
...
## Step 8: Connect the SDK to Mezon

Now that we have our access token, let's connect to Mezon!
...
## Step 9: Access Mezon Relationships
...
Hit play and then click the button. Once the OAuth flow is complete, you will see the status hit `Ready`. The console will tell you if setting rich presence was successful, and you can check your Mezon profile to see it!
...
Congratulations! You've successfully integrated the Mezon Social SDK into Unity. Let's review what you've accomplished:
...
You have successfully set up the Mezon Social SDK with Unity and authenticated with Mezon!You can now use the SDK to add more social features in your project.

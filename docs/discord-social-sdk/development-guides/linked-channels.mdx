...
Linked Channels let players connect in-game lobbies with Mezon text channels, enabling cross-platform chat between your game and Mezon servers.

When linked:
- In-game messages appear in the Mezon channel
- Mezon messages appear in your game
- Messages from your game show your game's name and icon in Mezon

### Prerequisites

Before implementing Linked Channels, make sure you have:

- [Set up the Mezon Social SDK](#DOCS_DISCORD_SOCIAL_SDK_GETTING_STARTED)
...
For a Mezon channel to be linkable with your game lobby, it must meet these requirements:

#### Channel Type and Settings

| Requirement  | Description                                 |
|--------------|---------------------------------------------|
| Channel Type | Must be a Mezon text channel in a server  |
| NSFW Status  | Cannot be marked as NSFW                    |
| Link Status  | Cannot be currently linked to another lobby |

#### Required Permissions

The user linking the channel must have these Mezon permissions:
...
Mezon allows all channels the user can access in a server to be linked in game, even if that channel is private to other server members. This means a user could choose to link a private "admins chat" channel (assuming they are an admin) in the game if they wanted.
...
To unlink a channel from a lobby, lobby members only require the `CanLinkLobby` flag to be set for them. They **do not** need to have any permissions on the Mezon side.
...
Channel linking happens entirely in the game. The user is never kicked out of the game to go to Mezon to set it up. So, at a high level, you will need to fetch the list of servers (also known as guilds) and channels the user can access, show them some UI to pick a channel, and then save that selection back to Mezon.
...
[`Client::GetUserGuilds`] will fetch the user's Mezon servers from our API. A list of `GuildMinimal` structs, including the guild ID and name, is returned on success.
...
- `isViewableAndWriteableByAllMembers` indicates if the channel in Mezon has restrictions on any members' or roles' ability to read or write to it (e.g. the channel may be considered private in some way). These read/write permissions are only enforced in the Mezon client. A player who may not be able to view the channel in-Mezon can read/write to it in-game as long as they are a lobby member. **It is important to notify the player performing the link that the channel contents may become exposed to players in-game who do not have access to the channel in Mezon.**
...
To test Linked Channels before building out a player interface, here are some steps you can follow to get things working in a prototype using the Mezon API:
...
3. Identify a Mezon server text channel that your `lobby_member` has the specified permissions enabled for (again, read/write and manage channels) and grab the channel's id.
...
[`Client::GetGuildChannels`]: https://mezon.com/developers/docs/social-sdk/classdiscordpp_1_1Client.html#adba1e5a83c219a9c4f6dab1657778017
[`Client::GetUserGuilds`]: https://mezon.com/developers/docs/social-sdk/classdiscordpp_1_1Client.html#aac1ec02df6074ed9213ce230e6a42fe1
[`Client::LinkChannelToLobby`]: https://mezon.com/developers/docs/social-sdk/classdiscordpp_1_1Client.html#a3114d58d50d4d2cb5752d95e121315d4
[`Client::UnlinkChannelFromLobby`]: https://mezon.com/developers/docs/social-sdk/classdiscordpp_1_1Client.html#a28f78a6fe46eb11eb54ee9b53fa94ffe
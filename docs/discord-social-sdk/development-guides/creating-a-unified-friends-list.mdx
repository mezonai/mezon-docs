...
A unified friends list combines both Mezon and game-specific relationships in one view. This guide will show you how to:
...
- Set up the Mezon Social SDK with [Getting Started Guide](#DOCS_DISCORD_SOCIAL_SDK_GETTING_STARTED)
...
The Mezon friend list is ultimately constructed from two entities: Relationships, and Users. You can query Relationships API to find everyone a user is a friend with, and the Users API to find the necessary extra information for rendering the list, such as whether they are online or not.

### Relationships

[Relationships](#DOCS_DISCORD_SOCIAL_SDK_DEVELOPMENT_GUIDES_MANAGING_RELATIONSHIPS) are how Mezon models friends, friend requests, and more. All relationships for the current user are loaded when the Client connects. Each relationship has a target user id, and a type, such as `Friend`, `PendingOutgoing`, or `Blocked`. The set of all relationships can be queried with [`Client::GetRelationships`].
...
Users are the Mezon users that are part of the relationships. The SDK provides a way to fetch a user by their ID, and the user object contains information such as their username, display name, avatar, and more.

### User Status & Rich Presence

Presence is how Mezon stores whether a user is currently online or not, as well as what activities they are currently doing (such as playing a game). The SDK gives you access to two types of status:

- Online Status: Online, Offline, Idle, etc.
- Rich Presence: Any activities associated with the current game (or application in Mezon parlance).  

> info
The SDK will only display activities associated with the current game, meaning you will not be able to see other game activities in Rich Presence, even if you can see them in the Mezon client.
...
void DisplayFriendsList(discordpp::Client& client) {
    ...
    for (auto& relationship: client.GetRelationships()) {
        ...
        // Provisional users don't have a Mezon icon shown next to them:
...
- We will add indicators for Mezon friends, game friends, and provisional users.
...
void DisplayFriendsList(discordpp::Client& client) {
    ...
    for (auto& relationship : client.GetRelationships()) {
        ...
        // Add Mezon friend indicator
        // In a real game, please use the official Mezon logo available in our design guidelines
        if (relationship.DiscordRelationshipType() == discordpp::RelationshipType::Friend) {
            str += " 👾";
        }
...
[`Client::SetUserUpdatedCallback`]: https://mezon.com/developers/docs/social-sdk/classdiscordpp_1_1Client.html#a3559f375165acedc6d6677ef599b3a4a
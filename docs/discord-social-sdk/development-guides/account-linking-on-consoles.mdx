...
This guide will walk you through implementing OAuth2 device authorization for console users, allowing them to log in to your game using their Mezon account. 
...
1. The game requests a device code from Mezon
2. To approve the authorization request, The user scans a QR code or enters a user code at `mezon.com/activate`.
3. Your game polls the Mezon API to exchange the device code for an access token.
...
Once you have the access token, you can authenticate the user with [`Client::UpdateToken`] and call [`Client::Connect`] to establish a connection with Mezon.
...
1. [Request a device code from Mezon](#DOCS_DISCORD_SOCIAL_SDK_DEVELOPMENT_GUIDES_ACCOUNT_LINKING_ON_CONSOLES/step-1-request-a-device-code-from-mezon)
...
### Step 1: Request a Device Code from Mezon

Your game must request a device and user code from the Mezon API.
...
```json
{
  ...
  "user_code": "<user code>",
  "verification_uri": "https://mezon.com/activate",
  "verification_uri_complete": "https://mezon.com/activate?user_code=<user code>",
  "expires_in": 300,
  "interval": 5
}
```
...
While the user completes the authorization request, your game must poll the Mezon Oauth2 token endpoint to exchange the device code for a valid access token.
...
Once the user completes authentication on Mezon, your game must exchange the device code for an access token.
...
Once exchanged, the API returns an access token to be used with [`Client::UpdateToken`] and [`Client::Connect`] to start using Mezon Social SDK features.
...
Now that you've successfully implemented account linking with Mezon on a console, you can integrate more social features into your game.
...
<Container>
  ...
  <Card title="Designing for Consoles" link="#DOCS_DISCORD_SOCIAL_SDK_DESIGN_GUIDELINES" icon="PaintPaletteIcon">
    Follow our design guidelines when integrating Mezon features into a console game.
  </Card>
</Container>
...
[`Client::CloseAuthorizeDeviceScreen`]: https://mezon.com/developers/docs/social-sdk/classdiscordpp_1_1Client.html#a4fcdd697702d086a8170a2d60a69acb8
...
[`Client::GetTokenFromDevice`]: https://mezon.com/developers/docs/social-sdk/classdiscordpp_1_1Client.html#a4a9c666b2d30bae0a16f5afd7ccee60d
[`Client::OpenAuthorizeDeviceScreen`]: https://mezon.com/developers/docs/social-sdk/classdiscordpp_1_1Client.html#af29a13acc992a75fc0870051ff68575b

...
Game Invites allow users to invite others to join their game session or party. This feature is available on the Mezon client and the Social SDK.

### Prerequisites

Before you begin, make sure you have:

- Set up the Mezon Social SDK with our [Getting Started guide](#DOCS_DISCORD_SOCIAL_SDK_GETTING_STARTED)
- Familiarized yourself with [Setting Rich Presence](#DOCS_DISCORD_SOCIAL_SDK_DEVELOPMENT_GUIDES_SETTING_RICH_PRESENCE)

> info
> Let's talk about the naming of some Mezon primitives first. Rich Presence, aka "Activity", can be thought of as the "current activity of a user" and is represented by the [`Activity`] class in the SDK and [in our gateway events](#DOCS_EVENTS_GATEWAY_EVENTS#activity-object). This is not to be confused with [Mezon Activities](#DOCS_ACTIVITIES_OVERVIEW), which are embedded games that can also set and display rich presence.
...
If we run our game, the Mezon client will show we are "In Competitive Match" on "Valhalla".
...
If we run our game, the Mezon client will show that we are "In Competitive Match" on "Valhalla" with more information about the party.
...
The `Join Secret` is a generic secret that you can use to [join a Mezon lobby](#DOCS_DISCORD_SOCIAL_SDK_DEVELOPMENT_GUIDES_MANAGING_LOBBIES), a game session, or something else. The game invite system is a way for players to share this secret value with other players - how you use it is up to you.

You will also need to set the supported platforms for joining the game so that the Mezon client can display the correct invite button for the user's platform.
...
Now your game supports rich presence with game invites! The Mezon client will show an invite button to your friends when they see your rich presence.

---

## Registering a Launch Command

Before we send a game invite, let's make sure that the Mezon client knows about your game and how to launch it.

When a user accepts a game invite for your game within Mezon, the Mezon client needs to know how to launch the game for that user. We have two ways to do this:

- Register a launch command for your game
- Register a Steam Game ID 

For desktop games, you should run one of these commands when the SDK starts up so that if the user tries to join from Mezon, the game can be launched for them.

### Registering a Launch Command 

[`Client::RegisterLaunchCommand`] allows you to register a command that Mezon will run to launch your game.
...
For Steam games, [`Client::RegisterLaunchCommand`] allows you to register what the Steam game ID. You should run this when the SDK starts up so that if the user tries to join from Mezon the game will be able to be launched for them.
...
1. Users can send game invites directly through the Mezon client.
2. You can programmatically send game invites on a user's behalf through the SDK.

### Sending Game Invites in the Mezon Client

Users can send game invites directly through the Mezon client. This feature is described in detail in the [Game Invites help center article](https://support.mezon.com/hc/en-us/articles/115001557452-Game-Invites).
...
1. Users can receive game invites directly through the Mezon client. 
2. Your game can receive game invites for a user programmatically through the SDK.

### Receiving Game Invites in the Mezon Client

Users can receive game invites directly in their DMs. This feature is described in detail in the [Game Invites help center article](https://support.mezon.com/hc/en-us/articles/115001557452-Game-Invites).
...
Use  [`Client::SetActivityJoinCallback`] to monitor for a user accepting a game invite, either in-game or in Mezon. Use the join secret to connect the players in your game.
...
- Another user can then see that Rich Presence on Mezon and request to join
...
When a player receives a game invite on mobile, Mezon must know how to launch your game. Game launching is handled through deep linking, which allows Mezon to pass the join information to your game.

### Setting Up Mobile Deep Links

1. Configure your deep link URL in the Mezon Developer Portal:
  - Go to your application's `General` tab
  - Enter your game's URL scheme (e.g., `yourgame://`)
  - Mezon will append `/_discord/join?secret=SECRETHERE` to your URL  

2. Tell Mezon which platforms can accept invites:
```cpp
...
1. The user receives and accepts an invite in Mezon
2. Mezon launches your game using your URL scheme:
   ```
...
[`Client::AcceptActivityInvite`]: https://mezon.com/developers/docs/social-sdk/classdiscordpp_1_1Client.html#ad12cf35065e4d2b303ee470af7c6ef37
...
[`Client::RegisterLaunchCommand`]: https://mezon.com/developers/docs/social-sdk/classdiscordpp_1_1Client.html#a024d7222931fdcb7d09c2b107642ecab
[`Client::SendActivityInvite`]: https://mezon.com/developers/docs/social-sdk/classdiscordpp_1_1Client.html#afc14e98fc070399895739da6d53efa60
[`Client::SetActivityInviteCreatedCallback`]: https://mezon.com/developers/docs/social-sdk/classdiscordpp_1_1Client.html#a3b4e37a222a8662506d763514774bedc
[`Client::SetActivityJoinCallback`]: https://mezon.com/developers/docs/social-sdk/classdiscordpp_1_1Client.html#a587d1c6d0352eba397c888987aa58418
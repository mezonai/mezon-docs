---
sidebar_label: Using with Mezon APIs
---
...
The Mezon Social SDK provides client-side functionality for integrating Mezon social features into your game. However, if you use a game backend, you should interact with Mezon's HTTP APIs for server-side operations when possible.

### Prerequisites

Before you begin, make sure you have:
- A Mezon application created in the Developer Portal
...
```
curl -X GET https://mezon.com/api/v10/users/@me \
...
def exchange_code(code, redirect_uri):
    ...
    response = requests.post('https://mezon.com/api/v10/oauth2/token', data=data)
...
def get_user_info(access_token):
    ...
    response = requests.get('https://mezon.com/api/v10/users/@me', headers=headers)
    return response.json()
```
...
You can use the [Mezon OpenAPI Spec](https://github.com/mezon/mezon-api-spec) to generate a client in your preferred backend language to make it easier to call the various APIs. A good tool for that is https://openapi-generator.tech/.
...
configuration = openapi_client.Configuration(host = "https://mezon.com/api/v10")

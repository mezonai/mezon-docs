...
Interactive features like commands and message components allows users to invoke an app natively within Mezon. When a user engages with one of your app's interactive features, your app will receive an [interaction](#DOCS_INTERACTIONS_RECEIVING_AND_RESPONDING/interaction-object).
...
There are different types of interactions in your app's toolbelt that can pick and choose to build engaging, interactive experiences in Mezon.

### Commands

**[Application commands](#DOCS_INTERACTIONS_APPLICATION_COMMANDS)** provide users a native way to invoke an app in Mezon. They often map to an app's core features or functionality.

![Command launcher in the Desktop client](overview-command-desktop.png)

When an app creates a command it can choose the command's type, which determines where it appears in the Mezon client and the metadata the app will receive when the command is invoked. There are three types for application commands:
...
- **User commands** are commands that related to a user in Mezon. They're accessed by right clicking on a user profile, then navigating to the "Apps" section.
...
**[Message components](#DOCS_INTERACTIONS_MESSAGE_COMPONENTS)** are interactive elements that can be included in the content of a message that your app sends in Mezon. 
...
- [Auto-populated select menus](#DOCS_INTERACTIONS_MESSAGE_COMPONENTS/select-menu-types) are a set of four different select components that are populated with contextual Mezon resources, like a list of users or channels in a server.
...
![Modals in the Mezon client](overview-modals.png)
...
A **Interactions Endpoint URL** is a public endpoint for your app where Mezon can send your app HTTP-based interactions. If your app is using [Gateway](#DOCS_EVENTS_GATEWAY)-based interactions, you don't need to configure an Interactions Endpoint URL.
...
1. Acknowledging `PING` requests from Mezon
...
In addition to ensuring your app validates security-related request headers at the time of saving your endpoint, Mezon will also perform automated, routine security checks against your endpoint, including purposefully sending you invalid signatures. If you fail the validation, we will remove your interactions URL and alert you via email and System DM.
...
After you have a public endpoint to use as your app's Interactions Endpoint URL, you can add it to your app by going to your [app's settings](https://mezon.com/developers/applications).

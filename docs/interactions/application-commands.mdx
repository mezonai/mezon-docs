...
Application commands are native ways to interact with apps in the Mezon client. There are 3 types of commands accessible in different interfaces: the chat input, a message's context menu (top-right menu or right-clicking in a message), and a user's context menu (right-clicking on a user).
...
| handler?                   | one of [command handler types](#DOCS_INTERACTIONS_APPLICATION_COMMANDS/application-command-object-entry-point-command-handler-types) | Determines whether the interaction is handled by the app's interactions handler or by Mezon                                                                                                                                                                                       | PRIMARY_ENTRY_POINT |
...
| DISCORD_LAUNCH_ACTIVITY | 2     | Mezon handles the interaction by launching an Activity and sending a follow-up message without coordinating with the app |
...
Global commands have inherent read-repair functionality. That means that if you make an update to a global command, and a user tries to use that command before it has updated for them, Mezon will do an internal version check and reject the command, and trigger a reload for that command.
...
url = "https://mezon.com/api/v10/applications/<my_application_id>/commands"
...
url = "https://mezon.com/api/v10/applications/<my_application_id>/guilds/<guild_id>/commands"
...
The interaction contexts for a command determines where in the Mezon client it can be used, and can be configured by setting the [`contexts` field](#DOCS_INTERACTIONS_APPLICATION_COMMANDS/application-command-object-application-command-structure) when creating or updating a command.
...
url = "https://mezon.com/api/v10/applications/<application_id>/guilds/<my_guild_id>/commands/<my_command_id>/permissions"
...
An Entry Point command serves as the primary way for users to open an app's [Activity](#DOCS_ACTIVITIES_OVERVIEW) from the [App Launcher](https://support.mezon.com/hc/articles/21334461140375-Using-Apps-on-Mezon#h_01HRQSA6C8TRHS722P1H3HW1TV).
...
- For `DISCORD_LAUNCH_ACTIVITY` (`2`), Mezon will handle the interaction automatically by launching the associated Activity and sending a message to the channel where it was launched.
...
Locale is determined by looking at the `X-Mezon-Locale` header, then the `Accept-Language` header if not present, then lastly the user settings locale.
...
> warn
> Apps with [discovery enabled](https://support-dev.mezon.com/hc/en-us/articles/9489299950487) (which is required to appear in the App Directory) cannot contain any age-restricted commands or content.
...
- an [age-restricted channel](https://support.mezon.com/hc/articles/115000084051-Age-Restricted-Channels-and-Content) or
- a DM with the app *after* [enabling age-restricted commands](https://support.mezon.com/hc/en-us/articles/10123937946007) within their User Settings.

Details about accessing and using age-restricted commands is in [the Help Center](https://support.mezon.com/hc/en-us/articles/10123937946007).

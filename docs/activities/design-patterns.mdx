...
- While most people use Mezon with small groups of friends. Consider experiences that are compelling alone, but better together!
...
### Respect user privacy
    ...
    - Mezon personal identifiers should only be populated into the Activity with user confirmation.
...
- This allows for easier drop-in drop-out behavior for the large portion of mobile users on Mezon.
...
- Mezon device usage is split between mobile and desktop platforms.

#### Mezon is a Global Audience
    - Consider supporting a multitude of languages and cultures to make your app more useable for all Mezon users.
...
- Take advantage of Mezon as a social platform: look for opportunities to offer users customizable skins, aesthetic themes, new avatars, etc.
...
### Mezon users who engage with voice are among our most engaged users
...
- Familiar design patterns can help to get people to participate in your Activity right away. Most Mezon users are extremely familiar with media browsing (Spotify, YouTube, Netflix, etc.) and will immediately understand your content if you follow these patterns.
...
- Remember that you need to contend with the Mezon Client itself for resources (CPU, RAM and GPU) as that client is still fully running while executing your Activity.
- Prioritize time-to-first-interaction.
- Minimize the amount of Web Assembly (WASM) as much as possible in your Activity.
    - Older iOS devices are especially affected by WebKit's optimization compilation pass of WASM that occurs during the first few minutes of usage of an Activity. To varying degrees, this will cause noticeable stutters, device thermal issues, and possibly degraded Mezon's AV quality during those early minutes.
...
- Test Mezon usernames handling, ensuring they display appropriately based on the chosen format (Mezon username or server identity).
...
- Enable users to cycle through Mezon buttons and fields on desktop by pressing TAB repeatedly, ensuring that the activity does not capture control.

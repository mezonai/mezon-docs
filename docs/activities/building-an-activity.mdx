...
# Building Your First Activity in Mezon

[Activities](#DOCS_ACTIVITIES_OVERVIEW) are web-based games and apps that can be run within Mezon. Activities are embedded in iframes within the Mezon client, and can be launched from the App Launcher or when responding to interactions.
...
In this guide, we'll be building a Mezon app with a basic Activity that handles user authentication and fetches data using the API.
...
- **[mezon/getting-started-activity](https://github.com/mezon/getting-started-activity)**, a project template to get you started
- **[@mezon/embedded-app-sdk](https://github.com/mezon/embedded-app-sdk)**, the SDK used to communicate between your app and Mezon when building Activities
...
Before getting started, you need to enable Developer Mode for your Mezon account if you don't already have it enabled. Developer Mode will allow you to run in-development Activities and expose resource IDs (like users, channels, and servers) in the client which can simplify testing. To enable Developer Mode:
1. Go to your **User Settings** in your Mezon client. On Desktop, you can access **User Settings** by clicking on the cogwheel icon near the bottom-left, next to your username.
2. Click on **Advanced** tab from the left-hand sidebar and toggle on `Developer Mode`.

## Step 1: Setting up the project

Before creating an app, let's set up our project code from the [`mezon/getting-started-activity`](https://github.com/mezon/getting-started-activity) repository.

Open a terminal window and clone the project code:

```
git clone git@github.com:mezon/getting-started-activity.git
```
...
Before creating our Mezon app, let's quickly install your project's frontend dependencies.
...
While it's not much at the moment, in the following steps we'll connect it to the backend services, make it runnable in Mezon, and power it up by populating it with data we pull from Mezon APIs.

<Collapsible title="Step 1 Checkpoint" icon="list" open>

By the end of Step 1, you should have:
- An understanding of what Mezon [Activities](#DOCS_ACTIVITIES_OVERVIEW) are
- Developer Mode enabled on your Mezon account
- Cloned the [sample project](https://github.com/mezon/getting-started-activity) to your development environment
...
<LinkButton to="https://mezon.com/developers/applications?new_application=true" color="brand">Create App</LinkButton>

Enter a name for your app, select a development team, then press **Create**.

> info
> **Development Team Access** <br />Launching a non-distributed Activity is limited to you or members of the developer team, so if you're collaborating with others during development, create a [developer team](https://mezon.com/developers/teams) and set it to the owner when you create the app.
...
Apps in Mezon can be installed to different **[installation contexts](#DOCS_RESOURCES_APPLICATION/installation-context)**: servers, user accounts, or both.
...
Click on **Installation** in the left sidebar, then under **Installation Contexts** make sure both "User Install" and "Guild Install" are selected. This will make sure users can launch our app's Activity across Mezon servers, DMs, and Group DMs.
...
1. **Client ID**: Copy the value for Client ID and add it to your `.env` file as **`VITE_CLIENT_ID`**. This is the public ID that Mezon associates with your app, and is almost always the same as your App ID.
...
The Embedded App SDK is a first-party SDK that handles the communication between Mezon and your Activity with [commands](#DOCS_DEVELOPER_TOOLS_EMBEDDED_APP_SDK/sdk-commands) to interact with the Mezon client (like fetching information about the channel) and [events](#DOCS_DEVELOPER_TOOLS_EMBEDDED_APP_SDK/sdk-events) to listen for user actions and changes in state (like when a user starts or stops speaking).
...
Back in our project's `client` directory from before (`getting-started-activity/client`), install the Embedded App SDK [via NPM](https://www.npmjs.com/package/@mezon/embedded-app-sdk):

```
npm install @mezon/embedded-app-sdk
```

This will add `@mezon/embedded-app-sdk` to `getting-started-activity/client/package.json` and install the SDK in your `node_modules` folder.

### Import the SDK in your Project

Once installed, we need to import it into our client code and instantiate it to start the handshake between our app and the Mezon client.
...
setupDiscordSdk().then(() => {
  console.log("Mezon SDK is ready");
});
...
document.querySelector('#app').innerHTML = `
  <div>
    <img src="${rocketLogo}" class="logo" alt="Mezon" />
    <h1>Hello, World!</h1>
  </div>
`;
```
...
> warn
> **Time to leave your browser behind**<br />Once you add the SDK to your app, you will **not** be able to view your app inside your web browser. In the next step, we will run your Activity inside of Mezon. In the next step, we will go over how to view your app in Mezon.
...
## Step 4: Running your app in Mezon

Let's ensure everything is wired up correctly, enable activities via the dev portal, and then run the Activity in Mezon.
...
Because Activities are in a sandbox enviornment and go through the Mezon proxy, you'll need to add a public URL mapping to serve your application and make external requests in your Activity. Since we're developing locally, we'll use the public endpoint we just set up.
...
When you enable Activities for your app, a [default Entry Point command](#DOCS_INTERACTIONS_APPLICATION_COMMANDS/default-entry-point-command) called "Launch" is automatically created. This [Entry Point command](#DOCS_INTERACTIONS_APPLICATION_COMMANDS/entry-point-commands) is the primary way for users to launch your Activity in Mezon.

By default, interactions with this command will result in Mezon opening your Activity for the user and posting a message in the channel where it was launched from. However, if you prefer to handle the interactions in your app, you can update the [`handler` field](#DOCS_INTERACTIONS_APPLICATION_COMMANDS/entry-point-handlers) or create your own. Additional details are in the Entry Point command [documentation](#DOCS_INTERACTIONS_APPLICATION_COMMANDS/entry-point-commands) and [development guide](#DOCS_ACTIVITIES_DEVELOPMENT_GUIDES/setting-up-an-entry-point-command).


### Running your Activity in Mezon

Now that we are pointing Mezon to our locally running app, we can launch the Activity in Mezon!

Navigate to your Mezon test server and, in any voice and or text channel, open the App Launcher where your in-development Activity should be present. If you don't see your Activity, you should try searching for its name.

Clicking on your app will launch your locally running app from inside Mezon!
...
We're looking pretty good so far, but we haven't wired up any Mezon functionality yet. Let's do that next.
...
- Successfully launched your Activity in Mezon
</Collapsible>
...
- [Back-end code](https://github.com/mezon/embedded-app-sdk-examples/blob/main/mezon-embedded-app-starter/packages/server/src/app.ts)
- [Front-end code](https://github.com/mezon/embedded-app-sdk-examples/blob/main/mezon-embedded-app-starter/packages/client/src/main.ts)
...
app.post("/api/token", async (req, res) => {
  ...
  const response = await fetch(`https://mezon.com/api/oauth2/token`, {
    method: "POST",
    headers: {
...
Before we call your backend activity server, we need to be aware of the Mezon proxy and understand how to avoid any Content Security Policy (CSP) issues.
...
setupDiscordSdk().then(() => {
  console.log("Mezon SDK is authenticated");
...
async function setupDiscordSdk() {
  ...
  // https://mezon.com/developers/docs/activities/development-guides#construct-a-full-url
...
Now if we relaunch our app, we'll be prompted to authorize with Mezon using the `identify`, `guilds`, and `applications.commands` scopes.
...
Now that we have authenticated our users, we can start interacting with contextual Mezon information that we can use in our application.
...
async function appendVoiceChannelName() {
  ...
  // the dm_channels.read scope which requires Mezon approval.
...
![Mezon Activities](activities/tutorial-channel-name.png)
...
async function appendGuildAvatar() {
  ...
  const guilds = await fetch(`https://mezon.com/api/v10/users/@me/guilds`, {
    headers: {
  ...
  if (currentGuild != null) {
    ...
    guildImg.setAttribute(
      'src',
      // More info on image formatting here: https://mezon.com/developers/docs/reference#image-formatting
...
![Mezon Activities](activities/tutorial-hero.png)
...
This is an intentionally simple example to get you started with the communication between your Activity and Mezon using the Embedded App SDK and APIs. From here, you can explore the [Activities documentation](#DOCS_ACTIVITIES_OVERVIEW) and other resources.
...
<Container>
  ...
  <Card title="Sample Activity Projects" link="https://github.com/mezon/embedded-app-sdk-examples" icon="ForumIcon">
     Try out the full range of Embedded App SDK features in the playground app, or explore some of the other examples
  </Card>
  <Card title="Mezon Developers" link="https://mezon.gg/mezon-developers" icon="GlobeEarthIcon">
    Join our community to ask questions about the API, attend events hosted by the Mezon platform team, and interact with other Activities developers
  </Card>
</Container>

...
Mezon apps can be installed to servers, users, or both. This guide will walk you through building a basic game integration app that is installable to both Mezon users and servers.
...
- **[GitHub repository](https://github.com/mezon/user-install-example)** where the code from this guide lives.
...
- **[Express](https://expressjs.com/)**, a popular JavaScript web framework we'll use to create a server where Mezon can send requests.
...
Before we dig in, you'll need the project code from the [sample app repository](https://github.com/mezon/user-install-example).
...
```
git clone https://github.com/mezon/user-install-example.git
```
...
With that out of the way, open your new project in the code editor of your choice, then we'll move ahead to setting up your Mezon app.
...
Before adding default install settings, we need to select Mezon Provided Link for the app's [install link](#DOCS_RESOURCES_APPLICATION/install-links). Under the **Install Link** section, select `Mezon Provided Link` from the dropdown if it isn't already selected (it should be by default). Once its selected, the **Default Install Settings** will appear.
...
> info
> Permissions for a bot user are very similar to permissions for other Mezon users. Details about permissions, and a list of available permissions is on the [Permissions](#DOCS_TOPICS_PERMISSIONS/permissions-bitwise-permission-flags) page.
...
Commands have two context fields that can be set when creating or updating a command which let you limit the supported install methods and surfaces in Mezon for that command:
...
| `/link`        | Link your game account to Mezon                      | `USER_INSTALL`                              | `BOT_DM`                             |
...
Now let's register your app's commands so you can see them in Mezon. In your project folder run:
...
After your new commands have been created, you can go into Mezon and look for the commands in the surfaces where we made them available:
...
To receive and handle interactive requests, we'll set up an **Interactions Endpoint URL**, which is a public URL where Mezon sends your app's interactions.
...
There should be some output indiciating your app is running on port 3000. Behind the scenes, our app is ready to handle interactions from Mezon, which includes verifying security request headers and responding to `PING` requests. We're skipping over a lot of the details in this tutorial, but details about preparing apps for interactions is in the [Interactions Overview](#DOCS_INTERACTIONS_OVERVIEW/preparing-for-interactions) documentation.
...
Go to your [app's settings](https://mezon.com/developers/applications) and on the **General Information** page under **Interaction Endpoint URL**, paste your new ngrok URL and append `/interactions` (it'll be something like `https://84c5df474.ngrok-free.dev/interactions`).

Click **Save Changes** and if all is well, your Interactions Endpoint URL should be verified by Mezon.
...
Back on the command line, our app is logging incoming requests from Mezon, so you can see what the request body for your command invocation looked like.
...
<Container>
  ...
  <Card title="Github repository" link="https://github.com/mezon/user-install-example">

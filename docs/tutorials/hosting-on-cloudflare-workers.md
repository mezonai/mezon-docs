...
# Hosting a Reddit API Mezon app on Cloudflare Workers

When building Mezon apps, your app can receive common events from the client as [webhooks](#DOCS_RESOURCES_WEBHOOK) when users interact with your app through interactions like [application commands](#DOCS_INTERACTIONS_APPLICATION_COMMANDS) or [message components](#DOCS_INTERACTIONS_MESSAGE_COMPONENTS).

Mezon will send these events to a pre-configured HTTPS endpoint (called an Interactions Endpoint URL in an app's configuration) as a JSON payload with details about the event.

This tutorial walks through building a Mezon app powered by [`r/aww`](https://www.reddit.com/r/aww) using JavaScript:

![Demo of Reddit API app](cloudflare-tutorial-demo.gif)

All of the code for this app can be found **[on GitHub](https://github.com/mezon/cloudflare-sample-app)**.

### Features and technologies used

- [Mezon Interactions API](#DOCS_INTERACTIONS_RECEIVING_AND_RESPONDING) (specifically slash commands)
- [Cloudflare Workers](https://workers.cloudflare.com/) for hosting
- [Reddit API](https://www.reddit.com/dev/api/) to send messages back to the user

---

## Creating an app on Mezon

To start, we'll create the app through the [Mezon Developer Dashboard](https://mezon.com/developers/applications):

- Visit https://mezon.com/developers/applications
...
> info
> For example, if my URL was `https://mezon.com/channels/123456/789101112`, the Guild ID is the first number—in this case **`123456`**.
...
│   ├── register.js           -> Sets up commands with the Mezon API
│   ├── server.js             -> Mezon app logic and routing
├── test
...
async function registerGlobalCommands() {
  const url = `https://mezon.com/api/v10/applications/${applicationId}/commands`;
...
When a user types a slash command, Mezon will send an HTTP request to a public endpoint. During local development this can be a little challenging, so we're going to use [a tool called `ngrok`](https://ngrok.com/) to create an HTTP tunnel.
...
Now head back to the Mezon Developer Dashboard, and update the `Interactions Endpoint URL` for your app:
...
export default {
  ...
  async fetch(request, env) {
    if (request.method === 'POST') {
      // Using the incoming headers, verify this request actually came from mezon.
...
All of the API calls from Mezon in this example will be POSTed to `/`. From here, we will use the [`mezon-interactions`](https://github.com/mezon/mezon-interactions-js) npm module to help us interpret the event, and to send results.

```js
/**
 * Main route for all requests sent from Mezon.  All incoming messages will
 * include a JSON payload described here:
 * https://mezon.com/developers/docs/interactions/receiving-and-responding#interaction-object
 */
...
> info
> In case you need to reference any of the code, you can find the repo [on GitHub](https://github.com/mezon/cloudflare-sample-app)
...
- Join the **[Mezon Developers server](https://mezon.gg/mezon-developers)** to ask questions about the API, attend events hosted by the Mezon API team, and interact with other developers.
